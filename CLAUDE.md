...
- **หลังแก้เสร็จ ต้องบอกวิธีทดสอบด้วยเสมอ** — หน้าไหนต้องเปิด, URL/พารามิเตอร์ไหนที่ทำให้เห็นเคสที่แก้ (เช่น `?date_range=ytd`, role ไหนใน Section Manager), และควรเห็นผลลัพธ์อะไรถึงจะถือว่าถูก เพราะโปรเจกต์นี้ไม่มี test suite/CI — การเปิดหน้าเว็บแล้วดูเองคือวิธีตรวจสอบเดียวที่มี (ดู "Running it")
- คอมเมนต์ในโค้ด: ให้น้อยที่สุดตามหลักเดิม (ดู "Conventions to follow") แต่จุดที่เป็น**สาเหตุที่แท้จริงของบั๊ก, กับดักที่เคยพังจริง, หรือเหตุผลที่เลือก pattern นี้แทนวิธีตรงไปตรงมากว่า** (เช่นทำไมใช้ `appendChild` แทน CSS order, ทำไม fallback ไป `total_amount` ตอน item rows ว่าง) ควรมีคอมเมนต์สั้นๆกำกับไว้ในโค้ดด้วย ไม่ใช่แค่อยู่ในบทสนทนา — เพราะเจ้าของโปรเจกต์ต้องอ่านโค้ดคนเดียวในอนาคตโดยไม่มีบทสนทนานี้ให้ย้อนดู
...
### Schema helper scripts: `scripts/add_indexes_factsales.sql`, `scripts/add_product_group_column.sql`

Standalone SQL files (run manually against the DB, not from PHP) for performance work identified while profiling `FactSales`/`fact_online_order_items` queries:
- `add_indexes_factsales.sql` — adds a covering nonclustered index on `FactSales(DateKey)` so Offline dashboard date-range queries don't need a key lookup back to the base table.
- `add_product_group_column.sql` — adds a `PERSISTED` computed column `product_group` on `fact_online_order_items`, precomputing the same `LIKE`-based category CASE expression `dashboard_online.php` currently builds at query time (`product_group_case()`), plus an index on it.

As of this writing, `dashboard_online.php` still evaluates the CASE expression at query time — the persisted column exists in the DB but the PHP hasn't been switched over to read it yet. If you do that migration, check every place `product_group_case()` (or equivalent inline CASE) is duplicated per SYSTEM_MAP.md's note on duplicated SQL helpers.
